...
public class KeyManagement {
     ...
     */
    private final HotKeyRuleBucket[] timeSlices;
    /**
     * 队列的总长度
     */
    ...
    public KeyManagement(int timeSliceSize, int windowSize, int timeSlice, TimeUnit timeUnit) {
        this(new HashFactory().getAllAlgorithms(),timeSliceSize, windowSize,timeSlice,timeUnit,DEFAULT_LOAD_FACTOR);
    }
     ...
     * @param loadFactor
     */
    public KeyManagement(List<HashStringAlgorithm> algorithms,int timeSliceSize, int windowSize, int timeSlice, TimeUnit timeUnit, float loadFactor) {
        ...
        this.timeSlices = new HotKeyRuleBucket[this.timeSliceSize];
        timeSliceMaxIndex = timeSliceSize - 1;
        threshold = (int) (this.windowSize * loadFactor);
        for (int i = 0; i < this.timeSliceSize; i++) {
            timeSlices[i] = new HotKeyRuleBucket(algorithms);
        }
    }
     ...
     * 增加count个数量
     */
    public synchronized boolean getAndSet(Object key) {
        //当前自己所在的位置，是哪个小时间窗
